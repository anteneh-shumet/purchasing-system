...
<?php include("list.php");?>
     ...
     <h2>Purchasing Tender Notices</h2>
...
If($connection){
		...
		If($sql){
             ...
             $sql2="select * from tender ORDER BY posted_date DESC"; 
...
echo"<td>".$row['posted_date'];
	echo "<td>".$row['subject']."</td>";
	echo "<td>".$row['content']."</td>";
	echo "<td>".$row['posted_date']."</td>";
	echo "<td>".$row['closing_date']."</td>";?>
	<form  id="email_form" action="posttendernotic.php"  method="post" style="align-items: center;"> <td>
...
<input type="hidden" name="closing_date" required value="<?php echo $d; ?>" />  
	<button type="submit" id="g" name="post">click to register </button></td></form><?php
	echo "</tr>";}
	echo "</table>";
	?>
